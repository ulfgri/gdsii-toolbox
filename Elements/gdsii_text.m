function [cet, width] = gdsii_text(str, etype, pos, height, ang, layer, dtype, pwidth, ptype)
%function [cet, width] = gdsii_text(str, etype, pos, height, ang, layer, dtype, pwidth, ptype)
%
% gdsii_text : renders a text with characters made either from
%              boundary or path elements
%
% INPUT:
% str   :  string to be drawn. The string can contain ASCII characters
%          33 to 126 and spaces (ASCII 32). char(127) encodes a lower
%          case Greek mu.
% etype :  a string with the element type to be used. Either 'boundary'
%          or 'path'
...
% layer :  (Optional) layer on which to draw the string. Default is 0.
% dtype :  (Optional) data type. Default is 0.
% pwidth : (Optional) path width when characters are rendered as paths.
%          Default is 1/20 (5%) of the height. Only used for path elements.
% ptype :  (Optional) path type to be used (see 'gds_element' constructor).
%          Options are 0,1,2,4. Default is 1, rounded path ends. 
%          Only use for path elements.
%
% OUTPUT:
...
% added the Greek mu, UG, July 2014
    ...
    persistent last_etype;

    % check arguments
    if nargin < 9, ptype = []; end
    if nargin < 8, pwidth = []; end
    if nargin < 7, dtype = []; end
    if nargin < 6, layer = []; end 
    if nargin < 5, ang = []; end;
    if nargin < 4, 
        error('gdsii_text: at least four arguments are required.');
    end
    if ~any( strcmp(etype,{'boundary','path'}) )
        error('gdsii_text: element type must be ''boundary'' or ''path''.');
    end
    
    % fill in default argument values
    if isempty(ptype), ptype = 0; end
    if isempty(pwidth), pwidth = 0.05*height; end
    ...
    if isempty(gdsii_fst) || ~strcmp(etype,last_etype)
        switch etype
         case 'boundary'
         ...
         otherwise
             error('gdsii_text: incorrect element type argument.');
        end
        last_etype = etype;
    end
...
% those of the Hershey sans font that was 
end
